...
// 表单验证规则
// Cron 表达式正则 (使用字符串构造避免构建问题)
const CRON_REGEX = new RegExp('^(\\*|([0-5]?\\d(-[0-5]?\\d)?)(,\\s*(\\*|([0-5]?\\d(-[0-5]?\\d)?)))*)\\s+(\\*|([01]?\\d|2[0-3])(-([01]?\\d|2[0-3]))?)(,\\s*(\\*|([01]?\\d|2[0-3])(-([01]?\\d|2[0-3]))?))*\\s+(\\*|([1-9]|[12]\\d|3[01])(-([1-9]|[12]\\d|3[01]))?)(,\\s*(\\*|([1-9]|[12]\\d|3[01])(-([1-9]|[12]\\d|3[01]))?))*\\s+(\\*|([1-9]|1[0-2])(-([1-9]|1[0-2]))?)(,\\s*(\\*|([1-9]|1[0-2])(-([1-9]|1[0-2]))?))*\\s+(\\*|[0-7](-[0-7])?)(,\\s*(\\*|[0-7](-[0-7])?))*$')
