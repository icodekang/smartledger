...
class ComprehensiveTester:
    ...
    def test_api(self, name, method, endpoint, expected_code=200, data=None, is_raw=False):
        """测试单个API"""
        try:
            if is_raw:
                if method == "GET":
    ...
    def create_test_data(self):
        ...
        print(f"\n  测试数据: {len(self.test_data['customers'])} 客户, {len(self.test_data['invoices'])} 发票")
        return self.test_data
    
    def run_comprehensive_tests(self):
        """运行全面测试"""
        print("\n" + "="*60)
        print("SmartLedger 全面API测试 V2")
        print("="*60)
        ...
        # 8. 财务报表 (需要customer_id和period参数)
        print("\n--- 财务报表模块 ---")
        customer_id = self.test_data["customers"][0] if self.test_data["customers"] else "test"
        tests = [
            ("费用明细表", "GET", f"/reports/expense-detail?customer_id={customer_id}&period=2024-01", 200),
            ("应收账款报表", "GET", f"/reports/accounts-receivable?customer_id={customer_id}&period=2024-01", 200),
            ("资产负债表", "GET", f"/reports/balance-sheet?customer_id={customer_id}&period=2024-01", 200),
            ("利润表", "GET", f"/reports/income-statement?customer_id={customer_id}&period=2024-01", 200),
            ("现金流量表", "GET", f"/reports/cash-flow?customer_id={customer_id}&period=2024-01", 200),
            ("科目余额表", "GET", f"/reports/subject-balance?customer_id={customer_id}&period=2024-01", 200),
        ]
        for name, method, endpoint, code in tests:
            result = self.test_api(name, method, endpoint, code)
            all_tests.append(result)
        ...
        tests = [
            ...
            ("健康检查", "GET", "/health", 200, None, True),  # 根路径
        ]
        for name, method, endpoint, code, *args in tests:
            is_raw = args[0] if args else False
            result = self.test_api(name, method, endpoint, code, is_raw=is_raw)
            all_tests.append(result)
            print(f"  {name}: {'✅' if result['status']=='PASS' else '❌'} ({result['code']})")
        ...
        tests = [
            ...
            ("插件市场", "GET", "/operations/plugins", 200),
        ...
        print("\n--- 移动端API模块 ---")
        tests = [
            ("H5仪表盘", "GET", "/mobile/h5/dashboard", 200),
            ("小程序仪表盘", "GET", "/mobile/miniapp/dashboard", 200),
            ("Portal仪表盘", "GET", "/mobile/portal/dashboard", 200),
        ]
        for name, method, endpoint, code in tests:
            result = self.test_api(name, method, endpoint, code)
            all_tests.append(result)
        ...
        # 12. AI功能
        print("\n--- AI智能功能模块 ---")
        invoice_id = self.test_data["invoices"][0] if self.test_data["invoices"] else "test"
        tests = [
            ("智能记账分析", "POST", "/ai/smart-booking/analyze", 200, {"bill_id": invoice_id}),
            ("异常检测", "GET", "/ai/anomaly/alerts", 200),
            ("智能洞察", "GET", "/ai/insights", 200),
            ("报表趋势", "GET", "/ai/reports/trend", 200),
        ]
        for name, method, endpoint, code, *args in tests:
            data = args[0] if args else None
            result = self.test_api(name, method, endpoint, code, data)
            all_tests.append(result)
            print(f"  {name}: {'✅' if result['status']=='PASS' else '❌'} ({result['code']})")
        ...
        print("\n--- 税务模块 ---")
        tests = [
            ("税务报表", "GET", f"/tax/reports?customer_id={customer_id}&period=2024-01", 200),
            ("税务设置", "GET", "/tax/settings", 200),
        ]
        for name, method, endpoint, code in tests:
            result = self.test_api(name, method, endpoint, code)
            all_tests.append(result)
        ...
        print("\n--- 安全模块 ---")
        tests = [
            ("安全设置", "GET", "/security/settings", 200),
            ("审计日志", "GET", "/security/audit-logs", 200),
        ]
        for name, method, endpoint, code in tests:
            result = self.test_api(name, method, endpoint, code)
            all_tests.append(result)
