...
class Step3Tester:
    ...
    def run_tests(self):
        ...
        # 银行管理
        tests = [("银行流水", "GET", "/bank-flows"), ("银行账户", "GET", "/bank-accounts"), ("银行列表", "GET", "/bank-accounts/banks")]
        for t in tests:
            r = self.test_api(*t)
        ...
        # 审核管理
        tests = [("待审核", "GET", "/audit/pending"), ("审核统计", "GET", "/audit/statistics"), ("我的任务", "GET", "/audit/my-tasks")]
        for t in tests:
            r = self.test_api(*t)
        ...
        # 报表
        tests = [("费用报表", "GET", f"/reports/expense-detail?customer_id={cid}&period=2024-01"), ("应收报表", "GET", f"/reports/accounts-receivable?customer_id={cid}&period=2024-01"), ("资产负债表", "GET", f"/reports/balance-sheet?customer_id={cid}&period=2024-01"), ("利润表", "GET", f"/reports/income-statement?customer_id={cid}&period=2024-01"), ("现金流量表", "GET", f"/reports/cash-flow?customer_id={cid}&period=2024-01"), ("科目余额表", "GET", f"/reports/subject-balance?customer_id={cid}&period=2024-01")]
        for t in tests:
        ...
        # 系统管理
        tests = [("系统角色", "GET", "/sys/roles"), ("系统日志", "GET", "/sys/logs"), ("系统配置", "GET", "/sys/config")]
        for t in tests:
            r = self.test_api(*t)
        ...
        # 运营管理
        tests = [("Webhook事件", "GET", "/operations/webhooks/events"), ("插件列表", "GET", "/operations/plugins"), ("Webhook列表", "GET", "/operations/webhooks")]
        for t in tests:
            r = self.test_api(*t)
        ...
        # 移动端
        tests = [("H5", "GET", "/mobile/h5/dashboard"), ("小程序", "GET", "/mobile/miniapp/dashboard"), ("Portal", "GET", "/mobile/portal/dashboard")]
        for t in tests:
            r = self.test_api(*t)
        ...
        # AI功能
        tests = [("异常规则", "GET", "/ai/anomaly/rules")]
        for t in tests:
            r = self.test_api(*t)
        ...
        # 税务
        tests = [("税务地区", "GET", "/tax/areas"), ("税务认证", "GET", "/tax/auths")]
        for t in tests:
            r = self.test_api(*t)
        ...
        # 安全 (使用正确的端点)
        tests = [("安全合规", "GET", "/security/compliance-status"), ("安全审计", "GET", "/security/audit-logs"), ("访问控制", "GET", "/security/access-control")]
        for t in tests:
            r = self.test_api(*t)
