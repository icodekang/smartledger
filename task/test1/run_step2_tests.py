...
def run_all_tests():
    ...
    results = {"name": "用户认证模块集成测试", "total": 4, "passed": 0, "failed": 0, "cases": []}
    ...
    # 错误密码
    try:
        resp = tester.session.post(f"{API_URL}/auth/login", data={"username": "admin", "password": "wrong"})
    ...
    try:
        ...
        if resp.status_code == 200:
            results["cases"].append({"id": "TC-INT-01-003", "name": "获取用户信息", "status": "PASS", "detail": "用户信息获取成功"})
            results["passed"] += 1
        else:
    ...
    try:
        ...
        if resp.status_code == 200:
            results["cases"].append({"id": "TC-API-01-001", "name": "列表查询", "status": "PASS", "detail": "200 OK"})
            results["passed"] += 1
        else:
            results["cases"].append({"id": "TC-API-01-001", "name": "列表查询", "status": "FAIL", "detail": f"状态码: {resp.status_code}"})
            results["failed"] += 1
    except Exception as e:
        results["cases"].append({"id": "TC-API-01-001", "name": "列表查询", "status": "ERROR", "detail": str(e)})
        results["failed"] += 1
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if items:
                resp = tester.get(f"/invoices/{items[0]['id']}")
                if resp.status_code == 200:
                    results["cases"].append({"id": "TC-API-01-002", "name": "详情查询", "status": "PASS", "detail": "票据详情获取成功"})
                    results["passed"] += 1
                else:
                    results["cases"].append({"id": "TC-API-01-002", "name": "详情查询", "status": "FAIL", "detail": f"状态码: {resp.status_code}"})
                    results["failed"] += 1
            else:
                results["cases"].append({"id": "TC-API-01-002", "name": "详情查询", "status": "SKIP", "detail": "无数据"})
        else:
            results["cases"].append({"id": "TC-API-01-002", "name": "详情查询", "status": "FAIL", "detail": f"状态码: {resp.status_code}"})
            results["failed"] += 1
    except Exception as e:
        results["cases"].append({"id": "TC-API-01-002", "name": "详情查询", "status": "ERROR", "detail": str(e)})
        results["failed"] += 1
    ...
    try:
        ...
        if resp.status_code == 404:
            results["cases"].append({"id": "TC-API-01-003", "name": "不存在票据返回404", "status": "PASS", "detail": "修复成功：返回404而非500"})
            results["passed"] += 1
        else:
            results["cases"].append({"id": "TC-API-01-003", "name": "不存在票据返回404", "status": "FAIL", "detail": f"状态码: {resp.status_code}"})
            results["failed"] += 1
    except Exception as e:
        results["cases"].append({"id": "TC-API-01-003", "name": "不存在票据返回404", "status": "ERROR", "detail": str(e)})
        results["failed"] += 1
    ...
    try:
        ...
        if resp.status_code == 200:
            results["cases"].append({"id": "TC-API-01-005", "name": "分页查询", "status": "PASS", "detail": "分页正常"})
            results["passed"] += 1
        else:
            results["cases"].append({"id": "TC-API-01-005", "name": "分页查询", "status": "FAIL", "detail": f"状态码: {resp.status_code}"})
            results["failed"] += 1
    except Exception as e:
        results["cases"].append({"id": "TC-API-01-005", "name": "分页查询", "status": "ERROR", "detail": str(e)})
        results["failed"] += 1
    ...
    try:
        ...
        if resp.status_code == 200:
            results["cases"].append({"id": "TC-INT-02-001", "name": "银行流水API", "status": "PASS", "detail": "API正常"})
            results["passed"] += 1
        else:
    ...
    try:
        ...
        if resp.status_code == 200:
            results["cases"].append({"id": "TC-INT-02-002", "name": "客户列表API", "status": "PASS", "detail": "API正常"})
            results["passed"] += 1
        else:
    ...
    try:
        ...
        if resp.status_code == 200:
            results["cases"].append({"id": "TC-INT-03-001", "name": "凭证列表API", "status": "PASS", "detail": "API正常"})
            results["passed"] += 1
        else:
    ...
    try:
        ...
        if resp.status_code == 200:
            results["cases"].append({"id": "TC-INT-03-002", "name": "会计科目API", "status": "PASS", "detail": "API正常"})
            results["passed"] += 1
        else:
    ...
    try:
        ...
        if resp.status_code == 200:
            results["cases"].append({"id": "TC-INT-03-003", "name": "凭证模板API", "status": "PASS", "detail": "API正常"})
            results["passed"] += 1
        else:
    ...
    try:
        ...
        if resp.status_code == 200:
            results["cases"].append({"id": "TC-INT-04-001", "name": "审计统计API", "status": "PASS", "detail": "API正常"})
            results["passed"] += 1
        else:
    ...
    try:
        ...
        if resp.status_code == 200:
            results["cases"].append({"id": "TC-INT-04-002", "name": "待办任务API", "status": "PASS", "detail": "API正常"})
            results["passed"] += 1
        else:
    ...
    try:
        resp = tester.get("/vouchers")
        if resp.status_code == 200:
            results["cases"].append({"id": "TC-INT-04-003", "name": "凭证状态查询", "status": "PASS", "detail": "API正常"})
            results["passed"] += 1
        else:
            results["cases"].append({"id": "TC-INT-04-003", "name": "凭证状态查询", "status": "FAIL", "detail": f"状态码: {resp.status_code}"})
            results["failed"] += 1
    except Exception as e:
        results["cases"].append({"id": "TC-INT-04-003", "name": "凭证状态查询", "status": "ERROR", "detail": str(e)})
        results["failed"] += 1
...
def generate_markdown_report(all_results):
    ...
    report = f"""# SmartLedger Step2 测试报告 (test1)
...
if __name__ == "__main__":
    print("="*60)
    print("SmartLedger Step2 API 测试")
    print("="*60)
    ...
    if results:
        ...
        with open("/root/.openclaw/workspace/smartledger/task/test1/01_step2_test_report.md", "w") as f:
            f.write(report)
        print("\n✅ Step2 测试报告已生成: task/test1/01_step2_test_report.md")
